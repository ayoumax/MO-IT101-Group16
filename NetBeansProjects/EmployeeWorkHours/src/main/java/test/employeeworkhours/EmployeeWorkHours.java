...
class EmployeeWorkHours {
    ...
    public static void main(String[] args) {
        String filePath = "employee_logs.xlsx";  // Change to actual file path
        ...
        exportToExcel(weeklyHours); // ✅ Export weekly hours to Excel
    }
    ...
    // 2. Process work logs and compute weekly hours per employee
    private static Map<String, Map<LocalDate, Double[]>> processWorkLogs(List<WorkLog> logs) {
        ...
        for (WorkLog log : logs) {
            LocalDate weekStart = log.date.with(DayOfWeek.MONDAY); // Get Monday of the week
    ...
    // 3. Display Weekly Hours Report (Formatted Table)
    ...
    // 4. Export Weekly Hours to Excel
    ...
    try (Workbook workbook = new XSSFWorkbook()) {
        ...
        headerRow.createCell(3).setCellValue("OT Hours");  // ✅ Added OT Hours Column
        ...
        for (var entry : weeklyHours.entrySet()) {
            ...
            for (var weekEntry : entry.getValue().entrySet()) {
                ...
                double otHours = weekEntry.getValue()[1];  // ✅ Fetch OT Hours
                ...
                row.createCell(3).setCellValue(otHours);  // ✅ Write OT Hours to Excel
            }
        }
